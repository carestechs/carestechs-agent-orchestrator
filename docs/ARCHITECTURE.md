...
### AD-5 — Durable run state, Postgres primary (JSONL opt-in for local dev)

| | |
|---|---|
| **Status** | **Implemented** as of FEAT-013 (T-271..T-275). The Postgres backend is the production default; `JsonlTraceStore` ships as opt-in `TRACE_BACKEND=jsonl` for local-dev ergonomics. |
| **Decision** | Runs, steps, policy decisions, webhook events, operator signals, effector calls, and executor calls persist to Postgres tables via SQLAlchemy async behind the `TraceStore` protocol. The four pre-existing kinds (`step`, `policy_call`, `webhook_event`, `operator_signal`) are written by their owning module (runtime / signal adapter / webhook router); `PostgresTraceStore.record_*` for those kinds is a no-op read-only seam. `EffectorCall` and `ExecutorCall` are written by the trace store itself. No in-memory-only mode is supported. |
| **Rationale** | JSONL gave us zero-setup durability for v1 launch; Postgres gives queryable history, multi-run aggregates, single backup/restore surface, and fits the profile's conventions. The migration was always anticipated (AD-5 v1 → v2). The protocol seam meant the swap was a composition-root change — runtime loops, FastAPI dependency tree, and the FEAT-004 streaming endpoint did not change. |
| **Constraints** | Trace writes remain append-only. The runtime loop depends only on the `TraceStore` protocol — never on either backend implementation directly (enforced by `tests/test_trace_postgres_is_isolated.py`). NEVER overwrite or truncate run history. Effector calls and executor calls are NOT folded into `open_run_stream` — they have dedicated reader paths to preserve the FEAT-004 endpoint contract byte-for-byte. |
| **Design doc** | [`docs/design/feat-013-trace-tail-mechanism.md`](design/feat-013-trace-tail-mechanism.md) — pins the tail mechanism (polling-only at 200 ms; no LISTEN/NOTIFY), the no-double-write rule, and the canonical `open_run_stream` query shape with per-source watermarks. |
...
### Lifecycle Agent Variants (FEAT-015)

The lifecycle agent ships as a family of variants — distinct agent definitions that share the same `lifecycle.v1` memory shape, the same engine workflows (`work_item_workflow`, `task_workflow`), and the same data-model contract, but differ in their **flow graph** and **executor bindings**. Each variant is a YAML in `agents/` plus a `register_lifecycle_v0X_<variant>(...)` bootstrap function in `src/app/modules/ai/executors/bootstrap.py`. Lifespan invokes one bootstrap per variant via `register_all_executors`, which routes by `agent_ref.startswith(...)`. Variant selection happens at run start via the `agentRef` parameter — no runtime branching.
...
| `lifecycle-agent@0.4.0-manual` | `register_lifecycle_v04_manual` | FEAT-015 | Operator-driven; four human checkpoints (`confirm_brief`, `confirm_tasks`, `confirm_plan`, `human_review_implementation`). Reuses v0.3.0's executor set via `register_lifecycle_v03(agent_ref=..., skip_review_implementation=True)`. |

The shared-helper-plus-overrides pattern (a sibling variant calls the prior-version helper under its own `agent_ref` and then registers variant-specific bindings on top) keeps drift to a minimum: every shared node is rooted in one place. The manual variant's `HumanExecutor` bindings each carry a `memory_patch_builder` from `executors/lifecycle_manual_patches.py` — at signal-delivery time the service-layer hook (`_deliver_to_human_dispatch`) calls the builder with the operator's payload and embeds the resulting `__memory_patch` on the dispatch envelope, which the runtime then merges into `RunMemory.data` via the standard `_write_state` pipeline.

### Lifecycle Subsystem (FEAT-006 + FEAT-008)

The lifecycle of work items and tasks is a separate axis from the agent runtime loop. AD-1 still holds for runs: the orchestrator drives the engine for agent execution. For **lifecycle state**, the relationship is inverted by FEAT-008 — the engine is the authoritative private backend and the orchestrator is a gateway + reactor. Both axes coexist; neither subsumes the other.

- **Lifecycle signal adapters** (`modules/ai/lifecycle/service.py`) — entry points for the 14 lifecycle signals (S1-S14, e.g. `approve-task`, `submit-implementation`). Each adapter forwards the transition to the engine, enqueues a `PendingAuxWrite` outbox row keyed on a fresh correlation id, commits, and returns 202. Adapters do not write `status` columns or aux rows under engine-present mode — those land via the reactor. An engine-absent fallback preserves the pre-FEAT-008 inline path for dev mode.
- **Lifecycle engine client** (`modules/ai/lifecycle/engine_client.py`) — JWT-authenticated HTTP client to `carestechs-flow-engine`. Encodes the orchestrator-side correlation id into `triggeredBy` (`orchestrator-corr:<uuid>`); the engine echoes it in the corresponding `item.transitioned` webhook so the reactor can match the original signal.
- **Lifecycle reactor** (`modules/ai/lifecycle/reactor.py`) — ingests `item.transitioned` webhooks at `/hooks/engine/lifecycle/item-transitioned` and runs an ordered pipeline: persist webhook event → materialize aux row from outbox → write status cache from `to_status` → consume the correlation context → dispatch effectors via the registry → **wake matching engine-mode dispatch** (FEAT-010) → fire W2/W5 derivations. Idempotent on `WebhookEvent.dedupe_key`. Status columns and aux rows (`Approval`, `TaskAssignment`, `TaskPlan`, `TaskImplementation`) are reactor-managed under engine-present mode. The wake-dispatch step (FEAT-010) is the resume hook for deterministic-mode `EngineExecutor` round-trips: it looks up the matching `Dispatch` by correlation id, calls `mark_completed`, and resolves the supervisor future. Wake runs *after* effectors so the resumed runtime sees effector-derived state, and *before* derivations so the runtime advances on the originating transition's outcome.
- **Effector registry** (`modules/ai/lifecycle/effectors/registry.py`) — first-class outbound surface. Effectors register at lifespan against transition keys (`task:approved->assigning`, `work_item:entry:open`, …); the reactor calls `fire_all` per webhook so registered effectors run end-to-end. Per-request `dispatch_effector` is the narrow exception for DI-bound effectors (e.g. GitHub Checks needing a per-request `GitHubChecksClient`); those carry a matching `no_effector` exemption. Every fire emits a `trace_kind="effector_call"` entry into `<trace_dir>/effectors/<entity_id>.jsonl`. A startup validator in `lifespan.py` rejects any declared transition that has neither a registered effector nor an exemption — silent transitions are a startup-fail.
- **Outbox** (`pending_aux_writes` table + `orchestrator reconcile-aux` CLI) — durable record of aux-write intent captured before the signal returns 202. If a webhook is lost, `reconcile-aux` queries the engine for the matching item, verifies the expected target state, and materializes the missed row. Idempotent; designed to be safe to run repeatedly. The outbox is a backstop, not the primary path.
- **Engine executor seam** (`modules/ai/executors/engine.py`, FEAT-010) — fourth executor sibling alongside `LocalExecutor`, `RemoteExecutor`, `HumanExecutor`. Where the others produce data, `EngineExecutor` advances **engine state**. Its dispatch persists a `PendingAuxWrite` outbox row + calls `FlowEngineLifecycleClient.transition_item` in one transaction, returns `dispatched`, and is woken by the reactor's wake-dispatch step when the engine's `item.transitioned` webhook echoes the matching correlation id. One executor instance per `(agent_ref, node_name)` binding, wired through `register_engine_executor` in `executors/bootstrap.py`. The deterministic runtime does not import `lifecycle/engine_client.py` — `EngineExecutor` consumes it via constructor injection only (`TYPE_CHECKING` import quarantine, verified by `tests/test_engine_executor_is_isolated.py`). This is the seam through which a `flow.policy: deterministic` agent can drive the authoritative engine state machine without re-implementing the FEAT-008 outbox + reactor pipeline inline.
- **Dispatch reconciler** (`modules/ai/executors/reconcile.py` + `orchestrator reconcile-dispatches` CLI, FEAT-010) — restart-safety backstop for orphan `Dispatch` rows. Engine-mode rows: query `FlowEngineLifecycleClient.get_item_state` and mark `failed` with one of `orchestrator_restart_engine_confirmed`, `orchestrator_restart_engine_did_not_transition`, or `orchestrator_restart_engine_unconfirmed` (the last is the conservative branch when no engine read is possible — outbox row is preserved for `reconcile-aux` to materialize later). Non-engine rows preserve the FEAT-009 conservative-cancel behavior bit-for-bit (`orchestrator_restart`). Idempotent.

The full architectural rationale lives in [`docs/design/feat-008-engine-as-authority.md`](design/feat-008-engine-as-authority.md), which supersedes the older `feat-006-rc2-architectural-position.md`.
...
- 2026-05-11 — FEAT-013 — AD-5 v2 (Postgres trace store) lands. The `TraceStore` protocol seam is unchanged; behind it `PostgresTraceStore` is now the production default and `JsonlTraceStore` is opt-in (`TRACE_BACKEND=jsonl`). Two new tables (`effector_calls`, `executor_calls`) cover the only kinds without a prior SQL home; the other four kinds (`step`, `policy_call`, `webhook_event`, `operator_signal`) are read from the existing tables and the corresponding `record_*` methods are no-ops in the Postgres backend (Decision 2 — no double-write). Tail mechanism is polling-only at 200 ms; the structural collision between Postgres `LISTEN` and the short-lived async-session discipline made LISTEN/NOTIFY a poor fit (Decision 1). The reader uses per-source watermarks so filter pushdown and watermark advance use the same column. Structural guard: `tests/test_trace_postgres_is_isolated.py` enforces that runtime / service / `trace.py` / `trace_jsonl.py` do not import `trace_postgres`. AD-5's status row flips to *implemented* in the same PR. Design doc: `docs/design/feat-013-trace-tail-mechanism.md`.
- 2026-05-13 — FEAT-015 — Lifecycle agent variants pattern documented. `lifecycle-agent@0.4.0-manual` ships as the first non-LLM-driven variant: same eight-stage skeleton as v0.3.0 with four `HumanExecutor` checkpoints inserted at the LLM→engine seams and a human reviewer replacing the LLM `review_implementation` node. Sibling variants register under distinct `agent_ref`s through dedicated `register_lifecycle_v0X_<variant>` bootstrap helpers; shared bindings factor through `register_lifecycle_v03(agent_ref=..., skip_review_implementation=...)`. New `HumanExecutor.memory_patch_builder` hook lets operator signal payloads mutate `RunMemory` at signal-delivery time via the existing `__memory_patch` envelope contract. No data-model, engine-side, or HTTP-shape changes — the variant rides on existing infrastructure.
- 2026-04-26 — FEAT-010 — Engine executor adapter is live. `EngineExecutor` (`modules/ai/executors/engine.py`) joins `LocalExecutor`, `RemoteExecutor`, `HumanExecutor` as the fourth sibling on the executor seam — its dispatch persists a `PendingAuxWrite` outbox row + calls `FlowEngineLifecycleClient.transition_item` in one transaction, returns `dispatched`, and is woken by the reactor's new wake-dispatch step. The reactor pipeline ordering at the wake point is canonical: `materialize aux → consume correlation context → fire effectors → wake dispatch → fire derivations` — effectors before wake so the resumed runtime sees effector-derived state, derivations after wake so the runtime advances on the originating transition's outcome. New `register_engine_executor` bootstrap helper in `executors/bootstrap.py`. New `orchestrator reconcile-dispatches` CLI (companion to FEAT-008's `reconcile-aux`) settles orphan `Dispatch` rows via engine state lookup; non-engine modes preserve FEAT-009 conservative-cancel. Import quarantine: `runtime_deterministic.py` does not transitively pull `lifecycle/engine_client.py` or `httpx`; the engine client is constructor-injected. `lifecycle-agent@0.1.0` LLM-policy + engine path is unchanged (regression bar — T-238). FEAT-011 (deterministic lifecycle port) is unblocked.
- 2026-04-25 — Deployment — Adapted ``docker-compose.prod.yml`` to the DevTools umbrella pattern (see ``../devtools-umbrella.md``). Embedded ``postgres`` service removed from the prod compose; the shared ``infra`` network's ``postgres`` container is used instead, with a per-project database (``agent_orchestrator``). Added ``docker/entrypoint.sh`` that runs ``alembic upgrade head`` before starting uvicorn so the project boots cold against an empty shared database. ``orchestrator-api`` is the canonical container name; sibling services reach it as ``http://orchestrator-api:8000``. Standalone path (``docker-compose.yml`` with bundled postgres) is preserved for solo-dev work.
- 2026-04-25 — FEAT-008 — Engine-as-authority is live. New "Lifecycle Subsystem" component subsection covers signal adapters, lifecycle engine client, reactor pipeline, effector registry, and the `pending_aux_writes` outbox. Aux rows (`Approval`, `TaskAssignment`, `TaskPlan`, `TaskImplementation`) are written by the reactor on correlation-matched webhook arrival, not inline at signal time. `work_items.status` / `tasks.status` are reactor-managed caches; `locked_from` / `deferred_from` are dropped (engine transition history is authoritative). Effector registry is the outbound surface — every declared transition either has a registered effector or an explicit `no_effector` exemption (startup validator enforces). `orchestrator reconcile-aux` CLI drains orphan outbox rows when webhooks are lost. Engine-absent fallback preserves the pre-FEAT-008 inline path for dev mode. Supersedes the FEAT-006 rc2-phase-1 / phase-2 framing in the entry below; the rc2 ADR carries a "Superseded by FEAT-008" banner.
