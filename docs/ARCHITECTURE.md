...
| **Active v2 design** | The Postgres backend is filed as [FEAT-013](work-items/FEAT-013-postgres-trace-store.md); the tail-mechanism, no-double-write rule, and `open_run_stream` query shape are pinned in [`docs/design/feat-013-trace-tail-mechanism.md`](design/feat-013-trace-tail-mechanism.md). AD-5 stays "JSONL-first then database" until the FEAT lands; the status row will flip to *implemented* in FEAT-013's docs-sweep task (T-281). |
