...
>
> **Relationship to FEAT-006.** FEAT-006 made the lifecycle flow deterministic by declaring transitions in the agent YAML. This FEAT carries that decision into the runtime loop *as the default*: when an agent's flow can be resolved deterministically, node selection is a pure function of `(current_node, memory, last_dispatch_result)` against the YAML, not an LLM call. **LLM-policy is preserved as an opt-in mode** for agents whose branches genuinely require model judgment that data on the dispatch result envelope cannot capture — the LLM-as-policy code path is not removed, it is repositioned from "default" to "opt-in." Most agents (the entire lifecycle flow included) have data-resolvable transitions and should not pay the LLM tax.
...
The orchestrator's runtime loop reduces to four steps **for agents declaring `flow.policy: deterministic`**: **resolve next node from the flow declaration → dispatch to the registered executor → wait for the result → record + advance**. No LLM call participates in step 1 of that path. Agents declaring `flow.policy: llm` keep the existing LLM-as-policy behavior — the path is preserved, repositioned from default to opt-in. Every artifact-producing step in any agent runs through a single executor contract — `dispatch(node, intake, context) → result via webhook` — with local handlers and remote endpoints both registered through the same registry. The orchestrator process itself produces no artifacts; flow decisions come from the YAML or, when the agent opts in, the LLM.
...
- A **deterministic flow resolver** that computes the next node from `(agent_flow_declaration, current_node, memory, last_dispatch_result)` as a pure function. Branches in the flow YAML (e.g. `review_implementation → [corrections, close_work_item]`) are resolved by data — either a memory predicate (`unplanned_tasks_remaining()` for the `generate_plan` self-loop) or a field on the last dispatch result (`result.verdict == "fail"` for the review branch). The resolver is the only thing that picks the next node *for agents declaring `flow.policy: deterministic`*; it never calls an LLM.
- An **agent-level `flow.policy` declaration** (`deterministic` | `llm`) selecting which path the runtime takes. Default is `llm` for backward compatibility — every existing agent (including `lifecycle-agent@0.1.0`) keeps its current behavior bit-for-bit until it opts in.
- A new deterministic-runtime module that implements **resolve → dispatch → await → record → advance** end-to-end: imports `FlowResolver`, never imports `core.llm`, dispatches via the executor registry, awaits via `RunSupervisor.await_dispatch`. The existing LLM-policy runtime is left untouched.
...
- **Forced removal of LLM-policy.** The LLM-policy code path is *not* deleted. It is repositioned from "default" to "opt-in" — agents declaring `flow.policy: llm` keep working unchanged. Permanent removal is a separate future decision once every active agent has migrated to `deterministic`.
...
- **AC-5 (revised)**: The runtime's *deterministic* path produces no artifacts and makes no LLM-as-policy calls. Every artifact-producing operation lives in an executor module outside the deterministic-runtime module. The LLM-policy path retains its existing call surface (it is, by definition, the *opt-in LLM mode* — the structural guard does not police it). Enforced by a structural test that asserts the deterministic-runtime module imports neither `core.llm` nor any executor handler module.
- **AC-8 (revised)**: For agents declaring `flow.policy: deterministic`, the `flow_resolver` module is a pure function with no I/O — given `(agent_flow_declaration, current_node, memory_snapshot, last_dispatch_result)` it returns either the next node name or a terminal sentinel. Verified by a unit test that exhaustively walks every transition in `lifecycle-agent@0.2.0` (including both branches of `review_implementation` and the `generate_plan` self-loop) without instantiating any LLM client. Agents declaring `flow.policy: llm` are exempt from this AC.
