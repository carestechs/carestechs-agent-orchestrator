...
<<<<<<< fix/llm-content-force-tool-choice-2
- 2026-05-01 — `LLMContentExecutor` robustness: (a) the executor now passes `tool_choice={"type": "tool", "name": <derived>}` to the provider, forcing the (single) tool call and eliminating the "model decides to chat instead" failure mode that surfaced as `provider_error: PolicyError("policy selected no tool")`. (b) `PolicyError` is now retried within the same `max_retries` budget as `ValidationError` — same class of "model misbehaved on this turn" failure, treated identically. The `LLMProvider` Protocol grew an optional `tool_choice` parameter (default `None` → providers choose `auto`); the LLM-policy `runtime.py` is unaffected because it doesn't pass `tool_choice`. Failed-envelope detail prefix changed from `result_schema_validation_failed` to `llm_content_retries_exhausted` (carries `last_error=validation_error: …` or `policy_error: …` so operators can tell which retry path exhausted).
=======
- 2026-05-01 — Task body expansion: `GenerateTasksTask` and `LifecycleTask` grew `description`, `acceptance_criteria`, `complexity`, `depends_on`, `files_hint` fields. `generate_tasks` now emits a real task spec (not just title + executor); `generate_plan` reads the current task's body via a new `prompt_context_loader` so the planner has structured input instead of re-deriving intent from the title alone. Memory schema is `lifecycle.v1`-compatible — additive change, no migration; old runs validate via field defaults.
>>>>>>> main
