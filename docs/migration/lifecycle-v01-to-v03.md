...
- 2026-05-01 — Runtime threads `taskId` from memory into every dispatch intake. Per-task LLM templates (`generate_plan`, `review_implementation`) and local executors (`correct_implementation`) read `taskId` from intake; before this fix the runtime only threaded `Run.intake` plus `runId`/`nodeName`, so task-scoped nodes hit `prompt_render_failed: missing template variable 'taskId'` on the first LLM call after `assign_task` woke (post wake-race fix). The runtime now reads `RunMemory.data['lifecycle.v1'].currentTaskId` (via `read_lifecycle_memory`, lazy-imported to keep the runtime's import-quarantine guard intact) and adds it to the dispatch intake via `setdefault` so caller-supplied `Run.intake.taskId` still takes precedence.
- 2026-05-01 — `CompositeLLMEngineExecutor` + `SequenceEngineExecutor` wake-race fix. Same split-tx pattern as `EngineExecutor`'s — pre-flight tx commits the outbox row(s) + `Dispatch.intake.correlation_id` before any HTTP call fires, compensating tx on engine failure marks dispatch failed and removes outbox row(s). For `SequenceEngineExecutor` specifically: all N hop outbox rows commit in tx 1, the last hop's correlation is what `Dispatch.intake.correlation_id` carries (since wake matches that), intermediate webhooks log a wake-miss WARNING (correctly — they shouldn't wake) but their outbox rows materialize/drop on arrival. Closes the deferred risk from PR #67's "out of scope" note — every engine-mode executor now wakes reliably regardless of webhook latency vs HTTP-response latency.
- 2026-05-01 — `EngineExecutor` wake-race fix. The runtime marks the dispatch `dispatched` before invoking the executor, but `intake.correlation_id` was only stamped *after* the executor returned. The engine fires its `item.transitioned` webhook back within tens of milliseconds — often before the originating `transition_item` HTTP call returns — so the reactor's wake-leg query (`Dispatch.intake['correlation_id'].astext == str(corr)`) silently missed every dispatch and the runtime timed out at 600s. `EngineExecutor.dispatch` now uses a split-tx: tx 1 commits the outbox row + `correlation_id`/`transition_key`/`engineItemId` onto `Dispatch.intake` *before* the HTTP call; tx 2 (compensating, on engine failure) marks the dispatch failed and removes the outbox row. Restart safety is preserved — `reconcile-dispatches` recovers if the orchestrator crashes between tx 1 commit and the HTTP call.
