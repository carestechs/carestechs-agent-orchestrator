...
- **Trace writes go through a protocol.** `modules/ai/service.py` emits traces via an interface. JSONL writer (v1) and SQLAlchemy writer (v2) are interchangeable implementations per AD-5. See `docs/design/feat-013-trace-tail-mechanism.md` for the Postgres backend's tail-mechanism choice (polling-only), the no-double-write rule for the four already-persisted kinds (`step`, `policy_call`, `webhook_event`, `operator_signal`), and the canonical `open_run_stream` query shape.
