...
uv run orchestrator reconcile-aux [--since=24h] [--dry-run]
uv run orchestrator reconcile-dispatches [--since=24h] [--dry-run]
...
│   │   ├── llm_content.py      — LLMContentExecutor (FEAT-011: LLM call wrapped as a local-mode dispatch; optional memory_patch_builder for standalone-node memory writes)
│   │   ├── engine_create.py    — EngineCreateExecutor (BUG-003: W1 create_item adapter; sibling of EngineExecutor for the engine's creation path)
│   │   ├── sequence.py         — SequenceEngineExecutor (BUG-004: chains N transitions on one engine target; wake on last hop)
│   │   ├── propose_tasks.py    — ProposeTasksExecutor (BUG-004: T1xN create_item + T2+T4 per task + W2; local-mode synchronous fanout)
│   │   ├── submit_implementation.py — SubmitImplementationExecutor (BUG-004: idempotent T9 with memory-flag guard)
...
- **Lifecycle agent variants are peers under distinct `agent_ref`s.** Each variant ships its own `agents/lifecycle-agent@X.Y.Z-<variant>.yaml` plus a `register_lifecycle_v0X_<variant>(...)` bootstrap function. The variant is selected at run start via the existing `agent_ref` parameter — no runtime branching. Shared bindings factor through the lowest-version helper (today: `register_lifecycle_v03(agent_ref=..., skip_review_implementation=...)`); variant-specific bindings (human checkpoints, alternative reviewers) layer on top. Current variants: `@0.3.0` (LLM-driven autonomous, FEAT-011), `@0.4.0-manual` (operator-driven, FEAT-015 + IMP-004 five-checkpoint variant). The manual variant carries a variant-specific top-level `assignments[taskId]` sidecar in `RunMemory.data` (written by `confirm_assignment`'s memory-patch builder, IMP-004); v0.3.0 memory is byte-unchanged. The `register_all_executors` dispatcher routes by `agent_ref.startswith(...)` to the matching bootstrap helper.
...
- **Work-item bodies live in the DB (FEAT-014).** Clients upload the brief inline via `intake.workItem = {id, kind, content?}` on `POST /api/v1/runs` (or directly via `POST /api/v1/work-items/upload` for import-without-running). The body is sha256-hashed and stored in `work_items.body_md` / `body_sha256`; briefs are immutable by sha — mismatched re-uploads return 409 `work-item-content-conflict`. The executor seam reads the body from the DB; `_handle_request_work_item_load` and the v0.3.0 `load_work_item` / `generate_tasks` LLM-content executors all key on `intake.workItem.id` (or legacy `workItemId`) and look up via `_load_work_item_body`. The orchestrator process never opens a file under `docs/work-items/` after FEAT-014 — verified by `tests/test_executors_dont_read_briefs.py`. The single exception is the T-288 deprecation shim in `service.py::_legacy_work_item_path_to_dto`, tagged `# DEPRECATED FEAT-014` for grep, removed in the next minor.
- **Code source lives on intake, read through `read_code_source` (IMP-005).** Every run's `(repo, baseBranch)` and optional `workBranch` are persisted on `Run.intake.codeSource` at run start (validated by `CodeSourceDto` in `app/modules/ai/schemas.py`). Executors MUST read it via `from app.modules.ai.executors.code_source import read_code_source` — direct access to `ctx.intake["codeSource"]` bypasses the memory-sidecar precedence and is a review blocker. Read order is fixed: memory sidecar's `workBranch` → intake's `workBranch` → None. Operator-supplied `workBranch` always wins; a producer executor (when one ships) writes `workBranch` to `RunMemory.data["codeSource"]` only when intake omitted it. No DB column — the field rides the existing `Run.intake` JSONB. Presence enforcement is gated by `LIFECYCLE_CODE_SOURCE_REQUIRED` (default `false` during the deprecation window; flip to `true` once all callers supply `codeSource` — the agent YAML's `required:` array follows in the same PR).
- **Trace writes go through a protocol.** `PostgresTraceStore` is the default backend (FEAT-013), `JsonlTraceStore` is opt-in local-dev (`TRACE_BACKEND=jsonl`), `NoopTraceStore` is the test default. The runtime loop never imports either implementation directly — only the protocol. The Postgres backend's `record_step` / `record_policy_call` / `record_webhook_event` / `record_operator_signal` are *no-op writes*: those rows are already persisted by the runtime / signal adapter / webhook router that owns them, so `PostgresTraceStore` reads them rather than re-writing. Effector calls and executor calls write to dedicated `effector_calls` / `executor_calls` tables. Tail mechanism is polling-only at 200 ms; LISTEN/NOTIFY was ruled out (structural collision with short-lived async sessions). See `docs/design/feat-013-trace-tail-mechanism.md` for the full rationale and the `open_run_stream` query shape.
...
- **Outbox + reconciliation backstop.** `pending_aux_writes` captures aux-write intent in the same transaction that commits the engine mirror. If the webhook is lost, the row stays orphaned. `uv run orchestrator reconcile-aux [--since=24h] [--dry-run]` queries the engine for the matching item, verifies the expected target state, and materializes the missed row. Idempotent — safe to re-run. Treat a growing orphan count as a health signal, not as a bug. Companion command `uv run orchestrator reconcile-dispatches [--since=24h] [--dry-run]` (FEAT-010 / T-235) settles orphan `Dispatch` rows the same way: engine-mode rows are queried against `FlowEngineLifecycleClient.get_item_state` and marked `failed` with one of `orchestrator_restart_engine_confirmed`, `orchestrator_restart_engine_did_not_transition`, or `orchestrator_restart_engine_unconfirmed` (the conservative branch when no engine read is possible); non-engine rows are cancelled with `orchestrator_restart` (FEAT-009 behavior preserved).
...
- **Executor seam is the producer surface (FEAT-009 + FEAT-010).** Every artefact-producing or state-advancing step in an agent declaring `flow.policy: deterministic` is a dispatch to a registered `Executor` — `local`, `remote`, `human`, or `engine`. Adding a new external integration = register an executor in `modules/ai/executors/bootstrap.py` against `(agent_ref, node_name)`. Local executors wrap a `(DispatchContext) -> Mapping[str, Any]` callable; remote executors POST to a configured URL and report back via `POST /hooks/executors/<id>`; human executors return `dispatched` immediately and wait for `POST /api/v1/runs/<id>/signals`; **engine executors** (FEAT-010) write a `PendingAuxWrite` outbox row + call `FlowEngineLifecycleClient.transition_item` in one transaction and are woken by the reactor's wake-dispatch step when the engine echoes the correlation id back via `/hooks/lifecycle/transitions`. Coverage is enforced at lifespan startup — every node either has a registered executor or an explicit `no_executor("≥10-char reason")` exemption. The deterministic runtime in `modules/ai/runtime_deterministic.py` imports neither `core.llm` nor any executor handler module (and crucially, not `lifecycle/engine_client.py`) — verified by `tests/test_runtime_deterministic_is_pure.py` and `tests/test_engine_executor_is_isolated.py`. See `docs/design/feat-009-pure-orchestrator.md` and `docs/design/feat-010-engine-executor.md`.
- **Reviewer binding is selected at bootstrap by `LIFECYCLE_REVIEWER` (IMP-003).** `register_lifecycle_v03` reads the setting and registers either the production `LLMContentExecutor` (default `llm-content`) or the `StubPassReviewerExecutor` (`stub-pass`, smoke / CI only — auto-approves every implementation). A future `remote` value is reserved for the external review service when it ships. Both bindings write `lifecycle.v1.reviewHistory` through the shared `_patch_review` builder, so the canonical memory shape is locked at one writer; the stub can never drift from the LLM path's contract. NEVER ship `stub-pass` to production.
- **LLM-content nodes register an `LLMContentExecutor`; the runtime loop never imports `core.llm`.** A deterministic agent that needs LLM-driven content production (briefs, plans, reviews) registers an `LLMContentExecutor` in `executors/bootstrap.py` per `(agent_ref, node_name)`. Each binding carries a `system_prompt` (loaded from `modules/ai/executors/prompts/<agent>/<node>.md`) and a `result_schema` (Pydantic v2 model the executor validates against). The LLM is a tool inside the executor, not at the runtime layer; the deterministic runtime loop's import-quarantine guard (`tests/test_runtime_deterministic_is_pure.py`) still holds. See `docs/design/feat-011-lifecycle-deterministic-port.md`.
- **Engine-bound nodes register an `EngineExecutor`, never call the engine inline.** A deterministic agent that needs to advance work-item or task state (W1–W6, T1–T12) wires a node via `register_engine_executor(registry, agent_ref, node_name, transition_key=..., to_status=..., lifecycle_client=..., session_factory=...)` in `executors/bootstrap.py`. The dispatch persists a `PendingAuxWrite` outbox row + calls `FlowEngineLifecycleClient.transition_item` in one transaction; the runtime suspends on the supervisor future; the reactor's wake-dispatch step (between effectors and W2/W5 derivations) resolves the future when the engine echoes the correlation id back. Restart safety lives in `executors/reconcile.py` and the `reconcile-dispatches` CLI. Inline `transition_item` calls in deterministic-mode executor handlers are forbidden — they bypass the outbox, the wake hook, and the reconciler. The legacy `lifecycle-agent@0.1.0` LLM-policy path through `lifecycle/service.py` is unchanged: it remains the only place that calls `transition_item` outside the engine executor.
...
- **Don't read a work-item brief from the filesystem in an executor (FEAT-014).** The body lives in `WorkItem.body_md`. Resolve via `intake.workItem.id` (or legacy `workItemId`) and call `_load_work_item_body(session_factory, external_ref=...)`. New `.read_text()` / `open()` against any path under `docs/work-items/` is a regression — caught by the structural guard in `tests/test_executors_dont_read_briefs.py`. The single allowed disk-read site is the T-288 deprecation shim in `service.py::_legacy_work_item_path_to_dto`, tagged `# DEPRECATED FEAT-014` for grep; do not add new tagged sites without updating the FEAT-014 plan and the guard's expected-count assertion.
