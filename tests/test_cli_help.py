...
_COMMAND_INVENTORY: list[tuple[str, list[str]]] = [
    ("orchestrator", ["run", "serve", "doctor", "reconcile-aux", "reconcile-dispatches", "runs", "agents"]),
...
class TestCommandInventory:
    @pytest.mark.parametrize(
        ("group", "expected"),
        [(group, expected) for group, expected in _COMMAND_INVENTORY],
        ids=[group for group, _ in _COMMAND_INVENTORY],
    )
    def test_help_lists_all_commands(self, group: str, expected: list[str]) -> None:
        args = ["--help"] if group == "orchestrator" else [group, "--help"]
        result = runner.invoke(main, args)
        assert result.exit_code == 0, result.output
        for cmd in expected:
            assert cmd in result.output, f"Expected `{cmd}` in `orchestrator {group} --help` output"
