...
1. **Node-to-engine-transition mapping table.** Every v0.3.0 node lists: which executor mode it registers (`engine` / `local` LLM-content / `human`), and — for engine-bound nodes — which `transition_key` (e.g. `work_item.W2`, `task.T6`) it binds. **First sub-step (gates the rest of T-250):** enumerate the **reachable transition set** by tracing a typical v0.1.0 lifecycle run end-to-end against `modules/ai/lifecycle/declarations.py` and the eight v0.1.0 tool handlers in `modules/ai/tools/lifecycle/`. Output: a list of every W1–W6 / T1–T12 transition that fires in a default run, plus a list of transitions defined in `declarations.py` but **not** reached (these stay un-mapped and are out of scope for v0.3.0). Without this, the mapping table either over-registers dead transitions or under-registers reachable ones; the AC-8 exhaustive branch walk depends on the reachable set being authoritative. Engine-bound (preliminary, confirm in the reachable-set step): at minimum `load_work_item` (W1), `generate_tasks` (W3), `assign_task` (T1), `generate_plan` (T2), `review_implementation` pass-branch (T7/T9 etc.), `correct_implementation` (T-correction), `close_work_item` (W4/W5/W6 sequence). Resolve the exact set in this doc — do not defer to T-253.
...
- [ ] Reachable-transition set is enumerated in writing: every W1–W6 / T1–T12 transition is listed as either *reached by a default v0.1.0 run* (in scope for v0.3.0 mapping) or *defined-but-unreached* (out of scope, justify briefly per transition).
- [ ] Mapping table covers all eight v0.3.0 nodes; every engine-bound node names its `transition_key` (or explicitly notes `local` / `human` mode); every reached transition appears in exactly one node's mapping.
